...
export const SupplyChain: React.FC<SupplyChainProps> = ({ onNavigateHome }) => {
  ...
  // Rate limit upsell
  const { handleError: handleRateLimit, banner: rateLimitBanner } = useRateLimit(
    () => { /* onUpgrade — will be passed via props in future, navigate to pricing */ window.dispatchEvent(new CustomEvent('taxreform:upgrade')); }
  );
  ...
  const handleSimulate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se atingiu rate limit, mostra banner de upsell e não faz fallback
      if (handleRateLimit(err)) { setLoading(false); return; }

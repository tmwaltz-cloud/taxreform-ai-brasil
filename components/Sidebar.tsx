...
import { LayoutDashboard, FileText, Activity, Link as LinkIcon, MessageSquareText, BookOpenCheck } from 'lucide-react';
import { UserRole } from '../types';

interface SidebarProps {
  currentView: 'dashboard' | 'supply-chain' | 'interpreter' | 'consultant' | 'accountant-guide';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, userRole }) => {
  const menuItems = [
    ...
    // Conditionally add Accountant Guide
    ...(userRole === UserRole.CONTADOR ? [{ id: 'accountant-guide', label: 'Guia do Contador 4.0', icon: BookOpenCheck }] : []),
  ...
  return (
    <aside className="w-20 md:w-64 bg-slate-900 text-white flex flex-col items-center md:items-stretch transition-all duration-300 z-10">
      <div className="h-16 flex items-center justify-center md:justify-start md:px-6 border-b border-slate-700">
        ...
        <span className="hidden md:block ml-3 font-bold text-lg tracking-tight">TaxReform<span className="text-brand-500">.ai Brasil</span></span>
      </div>
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center p-3 md:px-6 transition-colors duration-200
      ...
      <div className="p-4 border-t border-slate-700">
        <div className="hidden md:block text-xs text-slate-500 text-center">
          &copy; 2025 TaxReform.ai Brasil.<br/>Powered by Gemini.
        </div>
      </div>
    </aside>
  );
